...
export default function EtchOperations({ userId }: { userId: number }) {
  ...
  return <div className="et-app op-app">
    ...
    <header className="et-header"><Link className="et-brand" href="/dashboard"><b>SG</b> SemiGuard <small>OBSERVATION</small></Link><nav aria-label="제품 메뉴"><Link href="/training">시나리오 훈련</Link><Link href="/learn">8대 공정 학습</Link><Link href="/dashboard/legacy">기존 기록·계정</Link></nav></header>
    <main id="operations-main" className="et-main">
      ...
      {socialLinked ? <p role="status" className="op-message">소셜 계정 연결이 완료되었습니다. <Link href="/dashboard/legacy">기존 기록·계정 화면에서 확인</Link>할 수 있습니다.</p> : null}
      ...
      <p role="status" aria-live="polite" className="et-notice">{notice}</p><footer className="et-footer">가상 관찰 작업대 · 훈련 채점 없음 · 실제 팹 성능 미검증. 기존 4센서 대시보드의 기록과 계정 설정은 <Link href="/dashboard/legacy">보관 화면</Link>에서 확인할 수 있습니다.</footer>
    </main>
  </div>;
}
