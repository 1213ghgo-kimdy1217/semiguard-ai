...
export function SignalChart({ signal, until, marker, review = false, sampleStep = 1, sourceSamples }: { signal: EtchSignal; until: number; marker: number | null; review?: boolean; sampleStep?: number; sourceSamples?: ReturnType<typeof etchSamples> }) {
...
export function EvidenceSnapshot({ time, observedUntil, sourceRows }: { time: number; observedUntil: number; sourceRows?: ReturnType<typeof etchEvidence> }) {
...
export default function EtchTraining() {
  ...
  return <div className="et-app">
    ...
    <main className="et-main" id="etch-main">
      ...
      {stage === "home" ? <><p className="et-eyebrow">STEP 02 / CHOOSE YOUR PRACTICE</p><h1 ref={heading} tabIndex={-1}>연습 방식을 선택하세요.</h1><p className="et-lead">시나리오로 관찰 → 근거 기록 → 판단 → 복기를 따라가거나, 별도의 가상 신호 또는 기록 CSV를 자유롭게 살펴보세요. 실제 장비 연결이나 제어 기능은 없습니다.</p><div className="et-columns">
        ...
        <section className="et-panel et-navy"><Activity aria-hidden="true" /><p className="et-eyebrow">FREE OBSERVATION</p><h2>실시간 자유 분석</h2><p>가상 식각 챔버의 달라지는 센서 신호를 시간 제한 없이 비교하고 여러 시점의 근거 메모를 남기세요. 시나리오를 먼저 완료하지 않아도 이용할 수 있습니다.</p><p className="et-caption">실행마다 다른 가상 신호이며 Scenario 01의 정답 데이터와 분리됩니다. 실제 팹 장비 연결은 아직 지원하지 않습니다.</p><Link className="et-linkbutton" href="/live">자유 관찰 시작 <ArrowRight size={18} /></Link></section>
      </div></> : null}
