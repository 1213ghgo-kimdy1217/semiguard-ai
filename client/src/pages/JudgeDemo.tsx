...
type DemoCopy = {
  ...
  riskTitle: string; riskLabel: string; riskDescription: string; evidenceTitle: string; evidenceDescription: string; safeNextStep: string;
  ...
  mockTrace: string; evidenceLabel: string; currentEvidence: string; vibrationEvidence: string; scoreEvidence: string;
  futurePilot: string; futureBadge: string; futurePilotDescription: string;
  demoMode: string; demoModeDescription: string; reset: string; resetComplete: string; share: string; shareComplete: string; shareFailed: string;
  closeDemoMode: string; showDemoMode: string; close: string;
  methodTitle: string; methodDescription: string; methodFormula: string; methodBaseline: string; methodOutput: string; methodLimit: string; methodDisclaimer: string;
};

const copy: Record<DemoLanguage, DemoCopy> = {
  ko: {
    badge: "심사위원 읽기 전용 데모", title: "30초로 확인하는 SemiGuard AI 안전 분석", subtitle: "실제 설비·계정 데이터 없이, 위험 신호에서 점검 순서까지의 판단 흐름을 체험합니다.", readOnly: "읽기 전용", simulated: "시뮬레이션 데이터", languageLabel: "언어 선택",
    steps: ["위험 신호", "센서 근거", "권장 점검"], start: "데모 시작", previous: "이전", next: "다음 단계", complete: "핵심 흐름을 확인했습니다", riskTitle: "1. 위험 신호를 빠르게 확인합니다", riskLabel: "경고", riskDescription: "가상 진공 펌프의 위험도 점수가 상승했습니다. 이 화면은 교육·심사용 시뮬레이션이며 실제 설비 제어를 수행하지 않습니다.", evidenceTitle: "2. 센서 근거를 함께 확인합니다", evidenceDescription: "AI 판단은 수치만으로 고장을 단정하지 않습니다. 편차가 큰 센서를 먼저 보여 주고, 현장 점검과 매뉴얼 확인을 요청합니다.", safeNextStep: "안전한 다음 점검", actionTitle: "3. 권장 점검 순서를 확인합니다", actionDescription: "위험 신호는 담당자 판단을 보조합니다. 시스템이 자동으로 장비를 제어하거나 작업 결정을 대신하지 않습니다.", actionOne: "최근 30분의 전류·진동 추이를 확인합니다.", actionTwo: "관련 설비 매뉴얼과 직전 점검 이력을 대조합니다.", actionThree: "현장 담당자에게 보고하고 승인된 안전 절차에 따라 점검합니다.", timing: "3단계 · 약 30초", login: "정식 대시보드 로그인", loginShort: "로그인", privacy: "실제 사용자·설비 데이터는 이 데모에 포함되지 않습니다.", noControl: "설정 변경, 데이터 주입, 내보내기, 상담 기록 저장은 제공하지 않습니다.", sensorLabels: ["전류", "온도", "진동", "소음"],
    mockTrace: "시뮬레이션 30분 추이 · 12개 관측값", evidenceLabel: "센서 근거", currentEvidence: "기준 대비 +3.7 A", vibrationEvidence: "최근 30분 상승 추세", scoreEvidence: "위험도 67/100 · 경고", futurePilot: "실제 팹 파일럿 연동", futureBadge: "추후 지원 예정", futurePilotDescription: "협력 기관의 데이터 제공, IT/OT 보안 승인, 현장 담당자 협의가 완료된 경우에만 별도로 검토합니다.", demoMode: "데모 모드", demoModeDescription: "심사용 시뮬레이션 데이터 · 반복 시연 가능 · 실제 설비 제어 없음", reset: "데모 초기화", resetComplete: "데모를 첫 단계로 초기화했습니다.", share: "링크 복사", shareComplete: "현재 언어의 심사위원 데모 링크를 복사했습니다.", shareFailed: "링크를 복사하지 못했습니다. 브라우저 권한을 확인해주세요.", closeDemoMode: "데모 모드 배너 닫기", showDemoMode: "데모 모드 안내 다시 표시", close: "닫기", methodTitle: "탐지 방식과 검증 상태", methodDescription: "현재 위험 점수는 학습된 고장 분류 모델이 아니라, 4개 센서가 기준 범위에서 벗어난 정도를 합산하는 규칙형 계산입니다.", methodFormula: "계산: 센서별 |z-score| × 8점, 센서당 최대 25점, 4개 센서 합계 최대 100점", methodBaseline: "기준: 전류 5.0±0.5A · 온도 45±3°C · 진동 2.0±0.3mm/s · 소음 55±4dB", methodOutput: "출력: 0~100점과 정상·주의·경고·위험 4단계", methodLimit: "실제 팹 로그로 정확도·오탐·미탐·사전 경고 시간을 검증한 상태는 아닙니다.", methodDisclaimer: "Isolation Forest 라이브러리나 사전 학습된 모델은 현재 사용하지 않습니다. 과거 로그 비교와 읽기 전용 파일럿 검증은 향후 승인 후 진행할 범위입니다.",
  },
  en: {
    badge: "Read-only judge demo", title: "See SemiGuard AI safety analysis in 30 seconds", subtitle: "Experience the path from a risk signal to an inspection sequence without real equipment or account data.", readOnly: "Read only", simulated: "Simulated data", languageLabel: "Language",
    steps: ["Risk signal", "Sensor evidence", "Inspection steps"], start: "Start demo", previous: "Previous", next: "Next step", complete: "Core flow reviewed", riskTitle: "1. See the risk signal quickly", riskLabel: "Warning", riskDescription: "The risk score of a simulated vacuum pump has risen. This is an education and judging simulation; it never controls real equipment.", evidenceTitle: "2. Review the sensor evidence", evidenceDescription: "AI does not diagnose a failure from numbers alone. It highlights the largest deviations first and asks for on-site and manual verification.", safeNextStep: "Safe next step", actionTitle: "3. Review the recommended inspection order", actionDescription: "The risk signal supports the responsible operator. The system does not control equipment or replace a work decision.", actionOne: "Review the recent 30-minute current and vibration trend.", actionTwo: "Compare the relevant equipment manual and the last inspection record.", actionThree: "Report to the responsible operator and inspect under approved safety procedures.", timing: "3 steps · about 30 seconds", login: "Sign in to full dashboard", loginShort: "Login", privacy: "No real user or equipment data is included in this demo.", noControl: "Settings, data injection, export, and consultation history saving are unavailable.", sensorLabels: ["Current", "Temperature", "Vibration", "Noise"],
    mockTrace: "Simulated 30-minute trace · 12 observations", evidenceLabel: "Sensor evidence", currentEvidence: "+3.7 A from baseline", vibrationEvidence: "Upward trend over 30 minutes", scoreEvidence: "Risk score 67/100 · Warning", futurePilot: "Actual fab pilot integration", futureBadge: "Planned for future support", futurePilotDescription: "This is considered separately only after partner data access, IT/OT security approval, and on-site operator alignment.", demoMode: "Demo mode", demoModeDescription: "Judging simulation data · repeatable demo · no real equipment control", reset: "Reset demo", resetComplete: "Demo reset to the first step.", share: "Copy link", shareComplete: "Copied this language's judge-demo link.", shareFailed: "Could not copy the link. Check browser permission.", closeDemoMode: "Close demo mode banner", showDemoMode: "Show demo mode guidance", close: "Close", methodTitle: "Detection method and validation status", methodDescription: "The current risk score is not a trained fault-classification model. It is a rule-based calculation that adds each of four sensors' deviation from its baseline.", methodFormula: "Calculation: |z-score| × 8 per sensor, capped at 25 points each, capped at 100 points total", methodBaseline: "Baselines: Current 5.0±0.5A · Temperature 45±3°C · Vibration 2.0±0.3mm/s · Noise 55±4dB", methodOutput: "Output: score from 0 to 100 and four levels—normal, caution, warning, danger", methodLimit: "Accuracy, false alarms, misses, and early-warning time have not yet been validated with actual fab logs.", methodDisclaimer: "No Isolation Forest library or pre-trained model is currently used. Historical-log comparison and read-only pilot validation are future work after approval.",
  },
  ja: {
    badge: "審査員向け読み取り専用デモ", title: "30秒で確認するSemiGuard AI安全分析", subtitle: "実際の設備・アカウントデータなしで、危険信号から点検順序までの判断フローを体験できます。", readOnly: "読み取り専用", simulated: "シミュレーションデータ", languageLabel: "言語選択",
    steps: ["危険信号", "センサー根拠", "推奨点検"], start: "デモを開始", previous: "前へ", next: "次の段階", complete: "主要フローを確認しました", riskTitle: "1. 危険信号を素早く確認します", riskLabel: "警告", riskDescription: "仮想真空ポンプのリスクスコアが上昇しました。この画面は教育・審査用シミュレーションであり、実際の設備を制御しません。", evidenceTitle: "2. センサー根拠を確認します", evidenceDescription: "AIは数値だけで故障を断定しません。偏差の大きいセンサーを先に示し、現地点検とマニュアル確認を求めます。", safeNextStep: "安全な次の点検", actionTitle: "3. 推奨点検順序を確認します", actionDescription: "危険信号は担当者の判断を補助します。システムが設備を自動制御したり、作業判断を代替したりすることはありません。", actionOne: "直近30分の電流・振動の推移を確認します。", actionTwo: "関連設備マニュアルと直前の点検記録を照合します。", actionThree: "現場担当者へ報告し、承認された安全手順に従って点検します。", timing: "3段階・約30秒", login: "正式ダッシュボードにログイン", loginShort: "ログイン", privacy: "このデモには実際のユーザー・設備データは含まれません。", noControl: "設定変更、データ注入、エクスポート、相談履歴保存は提供しません。", sensorLabels: ["電流", "温度", "振動", "騒音"],
    mockTrace: "シミュレーション30分推移・12観測値", evidenceLabel: "センサー根拠", currentEvidence: "基準比 +3.7 A", vibrationEvidence: "直近30分の上昇傾向", scoreEvidence: "リスクスコア 67/100・警告", futurePilot: "実ファブのパイロット連携", futureBadge: "今後サポート予定", futurePilotDescription: "協力機関のデータ提供、IT/OTセキュリティ承認、現場担当者との協議が完了した場合にのみ個別に検討します。", demoMode: "デモモード", demoModeDescription: "審査用シミュレーションデータ・繰り返しデモ可能・実設備制御なし", reset: "デモをリセット", resetComplete: "デモを最初の段階にリセットしました。", share: "リンクをコピー", shareComplete: "現在の言語の審査員デモリンクをコピーしました。", shareFailed: "リンクをコピーできませんでした。ブラウザー権限を確認してください。", closeDemoMode: "デモモードバナーを閉じる", showDemoMode: "デモモード案内を再表示", close: "閉じる", methodTitle: "検知方式と検証状況", methodDescription: "現在のリスクスコアは学習済みの故障分類モデルではなく、4種類のセンサーが基準範囲から離れた程度を合算するルール型計算です。", methodFormula: "計算: センサーごとの |z-score| × 8点、センサーごと最大25点、4センサー合計は最大100点", methodBaseline: "基準: 電流 5.0±0.5A・温度 45±3°C・振動 2.0±0.3mm/s・騒音 55±4dB", methodOutput: "出力: 0～100点と正常・注意・警告・危険の4段階", methodLimit: "実ファブログによる精度、誤報、見逃し、早期警告時間の検証はまだ行っていません。", methodDisclaimer: "Isolation Forestライブラリや事前学習済みモデルは現在使用していません。過去ログ比較と読み取り専用パイロット検証は、承認後の今後の範囲です。",
  },
};
  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(6,182,212,0.22),transparent_38%),linear-gradient(135deg,#07111f,#0f172a_55%,#111827)] px-4 py-5 text-slate-100 sm:px-7 sm:py-8">
    <div className="mx-auto max-w-5xl">
      <header className="mb-6 flex flex-col gap-4 rounded-2xl border border-cyan-200/20 bg-slate-950/45 p-4 backdrop-blur sm:flex-row sm:items-start sm:justify-between sm:p-5">
        ...
        <div className="flex shrink-0 items-center gap-2" role="group" aria-label={text.languageLabel}>
          <label className="sr-only" htmlFor="judge-demo-language">{text.languageLabel}</label>
          ...
          <button type="button" onClick={() => void copyShareLink()} aria-label={text.share} title={text.share} className="h-9 rounded-lg border border-slate-600 bg-slate-900 px-3 text-xs font-bold text-slate-200 transition hover:border-cyan-300 hover:text-cyan-100 focus:outline-none focus:ring-2 focus:ring-cyan-200"><span aria-hidden="true">⧉</span><span className="hidden sm:ml-1 sm:inline">{text.share}</span></button>
          {!showDemoBanner && <button type="button" onClick={() => setShowDemoBanner(true)} title={text.showDemoMode} aria-label={text.showDemoMode} className="flex h-9 items-center rounded-lg border border-amber-200/60 bg-amber-300/15 px-2.5 text-xs font-bold text-amber-50 transition hover:bg-amber-300/25 focus:outline-none focus:ring-2 focus:ring-amber-200">● <span className="hidden sm:ml-1 sm:inline">{text.demoMode}</span></button>}
      ...
      <section className="mb-5" aria-labelledby="judge-demo-method-title">
        <details className="group overflow-hidden rounded-xl border border-cyan-200/25 bg-slate-950/55 shadow-lg shadow-cyan-950/15">
          <summary className="flex cursor-pointer list-none items-center justify-between gap-3 p-3.5 focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-cyan-200 sm:p-4">
            <div className="min-w-0">
              <h2 id="judge-demo-method-title" className="text-sm font-black text-cyan-50">{text.methodTitle}</h2>
              <p className="mt-1 text-xs leading-5 text-slate-300">{text.methodDescription}</p>
            </div>
            <span aria-hidden="true" className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-slate-600 bg-slate-900 text-slate-200 transition-transform duration-200 group-open:rotate-180">⌄</span>
          </summary>
          <div className="border-t border-slate-700/80 px-3.5 py-4 sm:px-4">
            <div className="grid gap-2 sm:grid-cols-3">
              {[text.methodFormula, text.methodBaseline, text.methodOutput].map((item) => <p key={item} className="rounded-lg border border-slate-700/80 bg-slate-900/75 p-3 text-xs leading-5 text-slate-200">{item}</p>)}
            </div>
            <p className="mt-3 rounded-lg border border-amber-300/35 bg-amber-300/10 p-3 text-xs font-semibold leading-5 text-amber-50">{text.methodLimit}</p>
            <p className="mt-2 text-[11px] leading-5 text-slate-400">{text.methodDisclaimer}</p>
          </div>
        </details>
      </section>
      ...
      <section className="rounded-2xl border border-slate-700/90 bg-slate-950/65 p-4 shadow-2xl shadow-cyan-950/20 sm:p-6" aria-labelledby="judge-demo-flow-title">
        ...
        <div id="judge-demo-step-panel" role="tabpanel" aria-labelledby={`judge-demo-step-tab-${step}`} className="min-h-[300px]">
          ...
          {step === 2 && <div className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
            ...
            <aside className="rounded-xl border border-cyan-300/30 bg-cyan-300/5 p-5"><p className="text-xs font-bold text-cyan-100">{text.evidenceLabel}</p><ul className="mt-4 space-y-3 text-xs leading-5 text-slate-300"><li className="rounded-lg bg-slate-900/75 p-3">• {text.sensorLabels[0]}: {text.currentEvidence}</li><li className="rounded-lg bg-slate-900/75 p-3">• {text.sensorLabels[2]}: {text.vibrationEvidence}</li><li className="rounded-lg bg-slate-900/75 p-3">• {text.scoreEvidence}</li></ul></aside>
          </div>}

          {step === 3 && <div className="grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
            <div className="rounded-xl border border-emerald-300/35 bg-emerald-300/5 p-5"><p className="text-xs font-bold text-emerald-100">{text.safeNextStep}</p><p className="mt-4 text-sm leading-6 text-slate-200">{text.actionDescription}</p><div className="mt-5 rounded-lg border border-slate-700 bg-slate-900/75 p-3 text-xs text-slate-400">{text.complete}</div><aside className="mt-3 rounded-lg border border-slate-700 bg-slate-950/50 p-3 opacity-70" role="note"><div className="flex items-center justify-between gap-2"><p className="text-[10px] font-bold text-slate-400">{text.futurePilot}</p><div className="group relative"><span tabIndex={0} aria-describedby="judge-demo-future-pilot-tooltip" className="block cursor-help rounded-full focus:outline-none focus:ring-2 focus:ring-slate-300"><button type="button" disabled aria-disabled="true" className="cursor-not-allowed rounded-full border border-slate-600 bg-slate-800 px-2 py-1 text-[9px] font-bold text-slate-400 grayscale">{text.futureBadge}</button></span><div id="judge-demo-future-pilot-tooltip" role="tooltip" className="pointer-events-none absolute bottom-full right-0 z-20 mb-2 w-56 translate-y-1 rounded-xl border border-slate-600/80 bg-slate-900/95 px-3 py-2 text-[10px] leading-4 text-slate-200 opacity-0 shadow-xl shadow-slate-950/50 transition duration-150 group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:translate-y-0 group-focus-within:opacity-100">{text.futurePilotDescription}</div></div></div><p className="mt-2 text-[10px] leading-4 text-slate-500">{text.futurePilotDescription}</p></aside></div>
