...
const copy: Record<DemoLanguage, DemoCopy> = {
  ko: {
    ...
    mockTrace: "시뮬레이션 30분 추이 · 12개 관측값", currentEvidence: "기준 대비 +3.7 A", vibrationEvidence: "최근 30분 상승 추세", scoreEvidence: "위험도 67/100 · 경고", futurePilot: "실제 팹 파일럿 연동", futureBadge: "추후 지원 예정", futurePilotDescription: "협력 기관의 데이터 제공, IT/OT 보안 승인, 현장 담당자 협의가 완료된 경우에만 별도로 검토합니다.",
  },
  en: {
    ...
    mockTrace: "Simulated 30-minute trace · 12 observations", currentEvidence: "+3.7 A from baseline", vibrationEvidence: "Upward trend over 30 minutes", scoreEvidence: "Risk score 67/100 · Warning", futurePilot: "Actual fab pilot integration", futureBadge: "Planned for future support", futurePilotDescription: "This is considered separately only after partner data access, IT/OT security approval, and on-site operator alignment.",
  },
  ja: {
    ...
    mockTrace: "シミュレーション30分推移・12観測値", currentEvidence: "基準比 +3.7 A", vibrationEvidence: "直近30分の上昇傾向", scoreEvidence: "リスクスコア 67/100・警告", futurePilot: "実ファブのパイロット連携", futureBadge: "今後サポート予定", futurePilotDescription: "協力機関のデータ提供、IT/OTセキュリティ承認、現場担当者との協議が完了した場合にのみ個別に検討します。",
  },
};
      ...
      <section className="rounded-2xl border border-slate-700/90 bg-slate-950/65 p-4 shadow-2xl shadow-cyan-950/20 sm:p-6" aria-labelledby="judge-demo-flow-title">
        ...
        <div role="tabpanel" className="min-h-[300px]">
          ...
          {step === 3 && <div className="grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
            <div className="rounded-xl border border-emerald-300/35 bg-emerald-300/5 p-5"><p className="text-xs font-bold text-emerald-100">SAFE NEXT STEP</p><p className="mt-4 text-sm leading-6 text-slate-200">{text.actionDescription}</p><div className="mt-5 rounded-lg border border-slate-700 bg-slate-900/75 p-3 text-xs text-slate-400">{text.complete}</div><aside className="mt-3 rounded-lg border border-amber-300/30 bg-amber-300/5 p-3" role="note"><div className="flex items-center justify-between gap-2"><p className="text-[10px] font-bold text-amber-100">{text.futurePilot}</p><span tabIndex={0} title={text.futurePilotDescription} aria-label={`${text.futurePilot}: ${text.futurePilotDescription}`} className="rounded-full border border-amber-200/35 bg-amber-300/10 px-2 py-1 text-[9px] font-bold text-amber-100 focus:outline-none focus:ring-2 focus:ring-amber-200">{text.futureBadge}</span></div><p className="mt-2 text-[10px] leading-4 text-slate-400">{text.futurePilotDescription}</p></aside></div>
