...
describe("chat fallback status contract", () => {
  ...
  it("shows an accessible multilingual fallback badge and allows the same question to be retried", () => {
    expect(dashboardSource).toContain("usedFallback: res.usedFallback ?? false,");
    expect(dashboardSource).toContain("isTemporaryServiceReply || res.usedFallback ? text.trim() : undefined");
