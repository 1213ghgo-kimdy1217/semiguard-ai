...
describe("dashboard statistics initial-loading contract", () => {
  it("distinguishes the first statistics load from an actual zero value", () => {
    expect(dashboardSource).toContain("const statsInitialLoading = periodOverviewQuery.isLoading && !periodOverviewQuery.data;");
  ...
  it("keeps KPI and savings cards accessible while statistics are pending", () => {
    ...
    expect(dashboardSource).toContain("periodOverviewQuery.isError || statsInitialLoading ? \"—\"");
  });
});
