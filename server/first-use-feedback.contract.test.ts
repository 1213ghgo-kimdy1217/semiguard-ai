...
describe("first-use feedback privacy and metrics contract", () => {
  ...
  it("retains historical feedback data without prompting in the observation workspace", () => {
    expect(dashboardSource).not.toContain("getFirstUseFeedback.useQuery");
    expect(dashboardSource).not.toContain('id="first-use-feedback-title"');
    expect(dashboardSource).not.toContain("feedbackResponseCount");
  });
});
