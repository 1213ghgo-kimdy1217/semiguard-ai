...
describe("dashboard and authentication loading status accessibility contract", () => {
  it("announces complete, busy loading states for lazy dashboard and protected-route waits", () => {
    const matches = appSource.match(/role="status" aria-live="polite" aria-atomic="true" aria-busy="true"/g) ?? [];
