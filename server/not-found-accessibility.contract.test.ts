...
const source = readFileSync(resolve(process.cwd(), "client/src/pages/NotFound.tsx"), "utf8");
...
describe("not found page accessibility contract", () => {
  it("uses the localized page title as the focused h1 and describes the recovery control", () => {
    ...
    expect(source).toContain('window.requestAnimationFrame(() => titleRef.current?.focus());');
    expect(source).toContain('h1 ref={titleRef} id="not-found-title" tabIndex={-1}');
