...
describe("authenticated user data isolation contract", () => {
  it("stores generated anomaly logs under the authenticated user instead of a fixed user", () => {
    ...
    expect(routerSource.match(/userId:\s*ctx\.user\.id/g)).toHaveLength(5);
  });
  ...
  it("scopes dashboard history and statistics to the authenticated user", () => {
    ...
    expect(routerSource).toContain("getPeriodDashboardOverview(\n          ctx.user.id,");
    ...
    expect(dbSource).toContain("export async function getPeriodDashboardOverview(userId: number");
  });
});
